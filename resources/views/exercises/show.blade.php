    ...
    <div class="div-ampliar mx-auto {{ $anchoCompleto ? '' : 'max-w-7xl' }} px-2 sm:px-6 lg:px-8 py-4"
        ...
        <div class="rounded-lg bg-azul-bonito py-4 px-4 mb-4">
            ...
            <div class="image-section rounded-lg w-400 flex items-center"
                @if($background_cuadros && $background_cuadros->image_path)
                style="background-image: url('{{ asset(Storage::url($background_cuadros->image_path)) }}')"
                @endif>

                {{-- Encabezado visible solo si $header_exercise está definido --}}
                @if($header_exercise && $header_exercise->image_path)
                {{-- Vista de encabezado para pantallas grandes y tablets --}}
                <div class="hidden sm:block">
                    <img src="{{ asset(Storage::url($header_exercise->image_path)) }}" alt="Encabezado"
                        class="ml-10" oncontextmenu="return false;">
                </div>

                {{-- Vista de encabezado para móviles --}}
                <div class="sm:hidden">
                    <img src="{{ asset(Storage::url($header_exercise->image_path)) }}" alt="Encabezado móvil"
            ...
            <div class="hidden sm:block">
                ...
                <div class="image-section rounded-lg w-400 h-300 flex items-center mt-1"
                    @if($background_cuaderno && $background_cuaderno->image_path)
                    style="background-image: url('{{ asset(Storage::url($background_cuaderno->image_path)) }}')"
                    @endif>

                    {{-- Mostrar imagen de desarrollo solo si existe --}}
                    @if (!empty($development_exercise) && $development_exercise->image_path)
                    <img src="{{ asset(Storage::url($development_exercise->image_path)) }}" alt="Ejercicio de desarrollo"
